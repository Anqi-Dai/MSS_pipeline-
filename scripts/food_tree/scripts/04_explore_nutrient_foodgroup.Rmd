...
patients_abs <- fgroup_relab %>% 
  filter(mrn %in% c(35454047, 35577168)) %>% 
  select(-daily_tol) %>% 
...
nutr_joined %>% 
  filter(mrn == 35580223) %>% 
  ggbarplot(x = 'DayRT', y  = 'nut_relab', fill = 'nutrient',size = 0.01,ylab = 'Relative abundance',
            legend = 'right',
...
# plot 
nutr_joined %>% 
  ggbarplot(x = 'DayRT', y  = 'nut_relab', fill = 'nutrient',size = 0.01,ylab = 'Relative abundance',legend = 'bottom',
            color = 'white', palette = 'lancet') +
  #geom_vline(data = engraf_df, aes(xintercept = dayEngra), color = 'red', size=2, linetype = 'dashed') +
  #geom_vline(data = TPN, aes(xintercept = dayRT), color = 'forestgreen', size=1, linetype = 'dotted') +
  facet_wrap(~ mrn) +
