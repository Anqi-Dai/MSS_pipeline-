...
```{r}
# gather all the csv contents info into a df 
fns_all <- list.files('../data/finalized/paired/p5day/', full.names = T)
...
for(pt in as.character(subset_pts)){
  cts_spilt_pt %>% 
    pluck(pt) %>% 
    write_tsv(str_glue('../data/finalized/p5day_{pt}_stool_relab_species.tsv'))
}

names_loop <- data_frame(
  name = as.character(subset_pts) 
) %>% 
    transmute(name = str_glue('p5day_{name}')) %>% 
  write_csv('../data/finalized/names_loop.csv', col_names = F)

# for the food
for(pt in as.character(subset_pts)){
  pair_pt_food %>% 
    pluck(pt) %>% 
    write_tsv(str_glue('../data/finalized/p5day_{pt}_diet_foodID_dehydrated_weight_per_pt.tsv'))
}
```
