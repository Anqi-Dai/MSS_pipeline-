...
ddd <- both %>% 
  select(Formulated_nutrition_bev,fg_sweets , mrn, sampleid, sdrt)  %>% 
  filter(Formulated_nutrition_bev > fg_sweets)
```

```{r}
# check why there is a dot that has so many nutritional drink consumption but not so many sweets
weird <- dtb %>% 
  filter(mrn == 35357493 & fdrt %in% c(3,4))
mean(c(28, 378, 100))
```
