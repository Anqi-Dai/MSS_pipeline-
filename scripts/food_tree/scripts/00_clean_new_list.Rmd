...
problematic <- uniq_food %>% 
  filter(!is.na(...7))

problematic %>% 
  write_csv('../data/source/more_than_one.csv')
```
...
```{r}
# a table with food nsc linking to a single food code
uniq_food %>% 
  select(Food_NSC:WWEIA.Category.Description) %>% 
  write_csv('../data/finalized/unique_food_nsc_with_food_code.csv')
```
...
## Keep cleaning and adding necesary columns
