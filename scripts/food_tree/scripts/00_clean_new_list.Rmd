...
check <- all %>% 
    ...
    #mutate(Calories_kcal = str_replace_all(Calories_kcal, '\\+$','')) %>% 
   mutate(Calories_kcal = as.numeric(Calories_kcal)) %>% 
  arrange(desc(Calories_kcal))



boxplot(check$Calories_kcal)
```

```{r}
# check some of the columns to see if it is drastically wrong
summary(All$Calories_kcal)

All %>% 
  ggboxplot(y = 'Calories_kcal')

All %>% 
  gghistogram(x = 'Calories_kcal', bins = 100)
