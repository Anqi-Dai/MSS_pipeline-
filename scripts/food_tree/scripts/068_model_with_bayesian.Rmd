...
In this script, the max and the mean are from the EN-less records.
...
diff_res <- maxs %>% 
  ...
  imap(~ gghistogram(data = .x, x = 'post_diff_', 
                     ...
                     title = str_glue('{.y}')) +
         xlim(-10, 5) +
         geom_vline(xintercept = 0, col = 'red', linetype = 'solid'))
...
pinterval <- both %>% 
  ...
  ggplot(aes(x = grp, y = coeff, color = grp)) +
  #stat_halfeye() +
  stat_pointintervalh(.width = c(.66, .95)) +
  scale_color_manual(values = c('#EC0000','#00468B')) +
  geom_hline(yintercept = 0, col = 'gray', linetype = 'solid', width = 4) +
  facet_wrap(item ~ .) +
  coord_flip() +
  ylim(-3,3)
...
both %>% 
  filter(grp == 'post') %>% 
  ggplot(aes(y  = item, x = coeff))  +
  stat_pointintervalh(.width = c(.66, .95)) 
 

pinterval
ggsave('../figs/paper/prior_post_halfeye_coeff.pdf', width = 10, height = 4)
```

```{r}
# try to make two ridges for each food group
# i don't know why in the ridges it actually crosses zero for the sweets group
