...
# calculate the perc that the value for mean_prior is between 0 and 4

prior_mean %>% 
  split(.$key) %>% 
  map_dfr(~ count(x = .x, value > 0 & value < max_log_div) %>% 
...
prior_all <- read_csv('../figs/paper/data/prior_ave_max_min_res_68.csv')
grps <- prior_all %>% 
  distinct(grp)
