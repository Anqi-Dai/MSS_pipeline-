...
prior_all %>%  write_csv('../figs/paper/data/prior_ave_max_min_res_68_with_EN.csv')
  ...
  map(function(char){
    prior_all %>% 
      ...
      gghistogram(x = 'prior_res', fill = 'grp', color = 'white', palette = 'lancet', xlab = 'diversity',
                  #add_density = TRUE, 
                  bins = 30,
                  title = str_glue('Prior check {char} (EN)'))
    })

g <- cowplot::plot_grid(prior_check_list[[1]],prior_check_list[[2]],prior_check_list[[3]],prior_check_list[[4]],prior_check_list[[5]],prior_check_list[[6]],prior_check_list[[7]],prior_check_list[[8]],prior_check_list[[9]],prior_check_list[[10]],
    ncol = 3, 
    o = 'hv',
    #labels = 'AUTO',
    axis = 'bltr') 
g
ggsave('../figs/paper/prior_check_10fg_EN.pdf', width = 10, height = 10, dpi = 300, plot = g)
...
posterior_model <- posterior_samples(model_w_EN) %>% 
...
post_all %>%  write_csv('../figs/paper/data/post_ave_max_min_res_68_with_EN.csv')
...
post_check_list <- fg_cha %>% 
  ...
  map(function(char){
    post_all %>% 
      filter(str_detect(grp, str_glue('ave_each|{char}'))) %>% 
      gghistogram(x = 'post_res', fill = 'grp', color = 'white', palette = 'lancet', xlab = 'logdiversity',
                  #add_density = TRUE, 
                  bins = 30,
                  title = str_glue('Post check {char} (EN)'))
    })

g_post <- cowplot::plot_grid(post_check_list[[1]],post_check_list[[2]],post_check_list[[3]],post_check_list[[4]],post_check_list[[5]],post_check_list[[6]],post_check_list[[7]],post_check_list[[8]],post_check_list[[9]],post_check_list[[10]],
    ncol = 3, 
    o = 'hv',
    #labels = 'AUTO',
    axis = 'bltr') 
...
ggsave('../figs/paper/post_check_10fg_wit_EN.pdf', width = 10, height = 10, dpi = 300, plot = g_post)
