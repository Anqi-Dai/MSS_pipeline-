...
final <- PTB %>% 
  select(age, sex, Disease = disease.simple, source,  intensity, ci) %>% 
  mutate(Disease = case_when(
    ...
    Disease == 'CML' ~ 'Chronic myeloid leukemia',
    Disease == 'Hodgkins' ~ "Hodgkin's lymphoma",
    Disease == 'Myeloma' ~ 'Myeloma', 
    Disease == 'AA' ~ 'Aplastic anaemia'
  )) %>% 
  mutate(source = case_when(
    source == 'unmodified' ~ "Unmodified bone marrow or PBSCs",
    source == 'cord' ~ "Cord blood",
    source == 'TCD' ~ "T-cell depleted PBSCs"
  )) %>% 
