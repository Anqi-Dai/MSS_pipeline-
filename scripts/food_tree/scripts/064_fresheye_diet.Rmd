...
dat %>% 
  filter(iRep <= 3) %>% 
  group_by(sampleid, order, fg_fruit, empirical) %>% 
  filter(!is.na(order)) %>% 
  summarise(med_irep = median(iRep)) %>% 
  ggscatter(x = 'fg_fruit', y = 'med_irep', facet.by = 'empirical',color = 'order',
            add = "reg.line",  # Add regressin line
           add.params = list(color = "blue", fill = "lightgray"), # Customize line
           conf.int = TRUE, # Add confidence interval
           cor.coef = TRUE, # Add correlation coefficient.
...
# to differentiate what abx?

s %>% 
  ggscatter(x = 'day0_g3', y = 'med_irep', facet.by = 'empirical',color = 'order',
            add = "reg.line",  # Add regressin line
           add.params = list(color = "blue", fill = "lightgray"), # Customize line
           conf.int = TRUE, # Add confidence interval
           cor.coef = TRUE, # Add correlation coefficient.
