...
```{r}
corvo <- res %>% 
  ggscatter( x = 'rho', y = 'y') +labs(y = '1 - P.adj', x ='Spearman correlation') +
  scale_color_manual(values = c("gray", "black")) +
  annotate("text", x = entero_cor, y = 0.9, label = "Enterococcus") +
  annotate("text", x = blautia_cor, y = 0.9, label = "Blautia") +
  xlim(-0.5, 0.8) +
  geom_segment(aes(x = entero_cor, y = 1, xend = -0.4, yend = 0.92),
                  arrow = arrow(length = unit(0.2, "cm"))) +
  geom_segment(aes(x = blautia_cor, y = 1, xend = 0.65, yend = 0.92),
                  arrow = arrow(length = unit(0.2, "cm"))) +
...
f3 <- plot_grid(corvo, entero_panel,mouse, labels = c('A', 'B', 'C'), label_size = 12, ncol = 1,
