...
p4d_scts %>% 
  write_csv('../data/cleaned_pair/p4d_scts.csv')

p1d_scts %>% 
  write_csv('../data/cleaned_pair/p1d_scts.csv')

p2d_scts %>%  
  write_csv('../data/cleaned_pair/p2d_scts.csv')

p3d_scts %>% 
  write_csv('../data/cleaned_pair/p3d_scts.csv')

p5d_scts %>% 
  write_csv('../data/cleaned_pair/p5d_scts.csv')

p1d %>% 
  write_csv('../data/cleaned_pair/p1d.csv')

p2d %>% 
  write_csv('../data/cleaned_pair/p2d.csv')

p3d %>% 
  write_csv('../data/cleaned_pair/p3d.csv')

p4d %>% 
  write_csv('../data/cleaned_pair/p4d.csv')

p5d %>% 
  write_csv('../data/cleaned_pair/p5d.csv')
...
matequal(a, b)   # FALSE
matequal(d, b) 
matequal(p1d %>% column_to_rownames('Food_code') %>% as.matrix(), p2d %>% column_to_rownames('Food_code') %>% as.matrix())

matequal(p4d %>% column_to_rownames('Food_code') %>% as.matrix(), p5d %>% column_to_rownames('Food_code') %>% as.matrix())

matequal(p3d_scts %>% column_to_rownames('taxa_species') %>% as.matrix(), p4d_scts %>% column_to_rownames('taxa_species') %>% as.matrix())

```

```{r}
p5d <- apply(stb_pNd, 1, function(Row){
  df = dtb %>%  
        filter(mrn == Row[['mrn']]) %>% 
        filter(fdrt %in% c(Row[['fp1d']], Row[['fp2d']], Row[['fp3d']], Row[['fp4d']], Row[['fp5d']])) %>% 
        group_by(Food_code) %>% 
        summarise(ave_dewt = mean(dehydrated_weight))
}) %>% 
  set_names(stb_pNd %>% pull(sampleid)) %>% 
