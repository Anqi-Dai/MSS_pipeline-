...
fg_pt <- cleaned_fg %>% 
  ...
  theme(legend.position = 'none') +
  theme(axis.text = element_text( size = 10),
         panel.background = element_rect(fill = "gray93",colour = "gray93",size = 0.5, linetype = "solid"),axis.title=element_text(size=10), 
        aspect.ratio=1/3.1)    
...
f2_bottom <- plot_grid(bottom_left,conditional9, rel_widths = c(1.2, 3) )

f2 <- plot_grid(f2_top,NA, f2_bottom,
                rel_heights = c(1,-0.09, .7),
                align = 'hv',axis = 'lrtb',
                 ncol = 1)
