...
conditional9 <- conditional %>%   
  ...
  theme_classic() +
  theme(legend.position = c(0.2, 0.07), legend.title = element_text(size = 8, face="bold"),
...
```
```{r}
# maybe plot them in two different plots and then combine together
row1fg <- c('Sweets','Grains', 'Milk','Eggs')
rows2fg <- rev( c('Vegetables','Oils', 'Fruits', 'Meats', 'Legumes'))
r1 <- conditional %>%   
  filter(shortname %in% row1fg ) %>% 
  ggplot() +
  geom_smooth(aes(x = effect1__, y = estimate__, ymin = lower__, ymax = upper__, fill = effect2__, color = effect2__),
              stat = "identity",
  ...
  ylim(0, 3.3) +
  labs(y = 'Predicted log(diversity)', x = '') +
  theme_classic() +
  theme(legend.position = 'none',
        aspect.ratio = 1, 
        strip.background = element_rect(color="white", fill=strip_color, size=1.5,linetype="solid"),
...
r2 <- conditional %>%   
  ...
  labs(y = 'Predicted log(diversity)', x = 'Food group consumed (grams)') +
  theme_classic() +
  theme(legend.position = c(0.2, 0.1), legend.title = element_text(size = 8, face="bold"),
        legend.text = element_text( size=8, face="bold"),
       legend.background=element_rect(fill = alpha("white", 0)),
      legend.key=element_rect(fill = alpha("white", 0)),
        aspect.ratio = 1, 
...
rawsweets <- META %>% 
  ...
  mutate(antibiotics = if_else(empirical == 'TRUE', 'exposed', 'not exposed')) %>% 
  ggscatter(x = 'fg_sweets', y = 'log_div', color = 'antibiotics',
           ...
           cor.coeff.args = list(method = "spearman",  label.sep = " \\n", cor.coef.name = c("rho"),p.accuracy = 0.01, r.accuracy = 0.01,label.x.npc = "left", label.y.npc = "bottom")) +
  scale_x_sqrt() +
  scale_y_sqrt() +
  scale_fill_manual(values = palette) +
  scale_colour_manual(values = palette) +
  facet_grid(~ antibiotics, labeller = 'label_both') +
  theme(aspect.ratio = 1, legend.position = 'none',strip.background = element_blank(), strip.text = element_blank(),
     axis.text=element_text(size=6), axis.title=element_text(size=axis_text_size)) 
rawsweets
...
```{r}
# row1_right <- plot_grid(NA,r1,
#                 #rel_widths = c(1,2),
#                  nrow = 2)
# row1 <- plot_grid(rawsweets,row1_right,
#                 #rel_widths = c(1,2),
#                  nrow = 1)
# # f2_bottom_1 <- plot_grid(rawsweets,r1,
# #                 rel_widths = c(1,2),
# #                  nrow = 1)
# f2_bottom <- plot_grid(row1,r2,
#                  nrow = 2)
```


```{r}
f2 <- plot_grid(f2_top,NA, conditional9,
                rel_heights = c(1,-0.15, .5),
                labels = c(NA,NA, 'F'), 
                #align = 'hv',axis = 'lrtb',
                 ncol = 1)
