...
Ymat <- matrix(rnorm(2000), ncol = 200, nrow = 10) 
library(vegan)
pss <- protest(Xmat, Ymat, permutations=999)
pss
```
...
both %>% 
  ...
  geom_text(data = real_data, mapping = aes(x = value, y = 5,label = value)) +
  facet_grid(type~.) +
  ggsave('../figs/procrustes_with_999_experiments.pdf',width = 7, height = 6)
