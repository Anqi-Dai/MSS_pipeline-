...
list_table_from_database('pathway')
get_table_from_database('tbl_humann2_pathways')
...
entero_box <- entero %>% 
  ggviolin(y = 'ave_irpe', add = 'boxplot') +
  theme_classic() +
   ylim(1, irep_highest) +
  theme(aspect.ratio=plot_ratio,
       axis.ticks.x=element_blank(),
...
fruit_time <- fruit %>% 
 ...
 # scale_y_log10(breaks = scales::trans_breaks("log10", function(x) 10^x), 
  ...
  theme(aspect.ratio=plot_ratio,#1/2.8
        plot.margin = margin(l=-0.8,unit="cm"),
        #axis.text.x=element_blank(),
        #axis.ticks.x=element_blank(),
        axis.text=element_text(size=axis_text_size),
...
p11_ver <- plot_grid(abx, NA,
          ...
          rel_widths = c(1, 1))
p11_ver %>% 
  write_rds('../data/075_p11_vertical5.rds')
 

ggsave('../figs/paper/075_irep_p11_vertical.pdf', width = 90,
      height = 220, units = 'mm', dpi = 400, plot = p11_ver)
```
```{r}
abx %>% write_rds('../data/075_abx.rds')
sweets_time %>% write_rds('../data/075_sweets_time.rds')
all_sweet_box %>% write_rds('../data/075_all_sweet_box.rds')
fruit_time %>% write_rds('../data/075_fruit_time.rds')
all_fruit_box %>% write_rds('../data/075_all_fruit_box.rds')
irep %>% write_rds('../data/075_irep.rds')
entero_box %>% write_rds('../data/075_entero_box.rds')
copies %>% write_rds('../data/075_copies.rds')
```
