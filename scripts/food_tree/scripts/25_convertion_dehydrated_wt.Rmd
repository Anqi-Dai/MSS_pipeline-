...
combined_dehy_clean %>% write_csv('../data/cleaned_diet_data/FINAL_97_with_code_all_foods.csv')
```

# convert the date to day relative to transplant
...
```{r}
combined_dehy_clean <- read_csv('../data/cleaned_diet_data/FINAL_97_with_code_all_foods.csv')


diet_pt <- patient_allo_ag %>% 
  filter(str_detect(indication, fixed('initial', ignore_case = T))) %>% 
  filter(mrn %in% combined_dehy_clean$MRN) 
    

diet_pt %>% 
  write_csv('../data/cleaned_patients/diet_patient_allo.csv')

# there is one patient I don't have transplant info!
setdiff(combined_dehy_clean$MRN, diet_pt$mrn)   

# that's cuz he's a very new transplant patients. this january!
```

## for now first just ignore that one patient that we don't have transplant info of

```{r}
combined_final <- combined_dehy_clean %>% 
  rename(mrn = MRN) %>% 
  inner_join(diet_pt %>% select(mrn, hct), by = "mrn") %>% 
