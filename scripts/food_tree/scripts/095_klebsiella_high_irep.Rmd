...
```{r whole_f3}
left <- align_plots(f3a, fruit_time, align = 'v', axis = 'l')

top <- plot_grid(left[[1]], f3b, labels = c('A', 'B'),rel_widths  = c(1, 1) )

bottom <- plot_grid(  left[[2]],violin6,
                  nrow = 1, labels = c('C','D'), rel_widths  = c(1, 0.5), align = 'hv')

test <- plot_grid(f3a,bottom,
                 rel_heights = c(1,1),
                 nrow = 2)

f3 <-  plot_grid(top,bottom,
                 rel_heights = c(1,1),
                 nrow = 2)
...
```{r}
# building up the plots one column and then another column
f3a <- read_rds('../data/094_f3a.rds')
left <- align_plots(f3a, abx,sweets_time,  align = 'v', axis = 'l')
row3 <- plot_grid(  left[[3]],violin6,
                  nrow = 1, labels = c('C','D'), rel_widths  = c(1, 0.5), align = 'hv')
