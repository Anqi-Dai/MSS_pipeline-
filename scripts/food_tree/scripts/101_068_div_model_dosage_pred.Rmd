...
mean_max_fruit <- pmap(test_coeffb, function(b_fg_fruit,b_fg_meat,b_fg_milk,b_fg_oils,b_fg_egg,b_fg_grain,b_fg_sweets,b_fg_legume,b_fg_veggie,b_intensityreduced){
  ...
  mutate(sigma = test$sigma) %>% mutate(div_mean_max_fruit = exp(mean_max_fruit))
...
mean_max_sweet <- pmap(test_coeffb, function(b_fg_fruit,b_fg_meat,b_fg_milk,b_fg_oils,b_fg_egg,b_fg_grain,b_fg_sweets,b_fg_legume,b_fg_veggie,b_intensityreduced){
  ...
  mutate(sigma = test$sigma)%>% mutate(div_mean_max_sweet = exp(mean_max_sweet))
...
mean_max_veggie <- pmap(test_coeffb, function(b_fg_fruit,b_fg_meat,b_fg_milk,b_fg_oils,b_fg_egg,b_fg_grain,b_fg_sweets,b_fg_legume,b_fg_veggie,b_intensityreduced){
  ...
  mutate(sigma = test$sigma)%>% mutate(div_mean_max_veggie = exp(mean_max_veggie))
```
...
sampled_div_fruit <- mean_max_fruit %>% 
  select(sigma, div_mean_max_fruit) %>% 
  pmap(function(sigma, div_mean_max_fruit){
    rnorm(500, mean = div_mean_max_fruit, sd = sigma)
  }) %>% set_names(seq(1, 100)) %>% 
  bind_rows(.id = 'id') %>% 
  gather()

sampled_div_sweet <- mean_max_sweet %>% 
  select(sigma, div_mean_max_sweet) %>% 
  pmap(function(sigma, div_mean_max_sweet){
    rnorm(500, mean = div_mean_max_sweet, sd = sigma)
  }) %>% set_names(seq(1, 100)) %>% 
  bind_rows(.id = 'id') %>% 
  gather()

sampled_div_veggie <- mean_max_veggie %>% 
  select(sigma, div_mean_max_veggie) %>% 
  pmap(function(sigma, div_mean_max_veggie){
    rnorm(500, mean = div_mean_max_veggie, sd = sigma)
  }) %>% set_names(seq(1, 100)) %>% 
  bind_rows(.id = 'id') %>% 
  gather()
```
...
mean_all <- pmap(test_coeffb, function(b_fg_fruit,b_fg_meat,b_fg_milk,b_fg_oils,b_fg_egg,b_fg_grain,b_fg_sweets,b_fg_legume,b_fg_veggie,b_intensityreduced){
  ...
  bind_rows(.id = 'ID') %>% 
  gather('key','mean_all')%>% 
  mutate(sigma = test$sigma)%>% mutate(div_mean_all = exp(mean_all))

sampled_div_mean_all <- mean_all %>% 
  select(sigma, div_mean_all) %>% 
  pmap(function(sigma, div_mean_all){
    rnorm(500, mean = div_mean_all, sd = sigma)
  }) %>% set_names(seq(1, 100)) %>% 
  bind_rows(.id = 'id') %>% 
  gather()
key
...
all_diff <- bind_cols(
  sampled_div_mean_all = sampled_div_mean_all$value,
  sampled_div_fruit = sampled_div_fruit$value,
  sampled_div_sweet = sampled_div_sweet$value,
  sampled_div_veggie = sampled_div_veggie$value
) %>% 
