...
predicted_ <- predict(mod102,newdata=prediction_subset,allow.new.levels=TRUE)

tibble(
  observed = prediction_subset$simpson_reciprocal,
  predicted = predicted_,
  mrn = prediction_subset$mrn
) %>% 
  ggscatter(x = 'observed', y = 'predicted', color = 'mrn',
