...
what if low fruit eaters had eaten a lot of fruit or what if high fruit eaters had eaten less fruit, then compare the predictions with the real vs the swabbed diet
...
high_eater_swab_both <- tibble(
  real = tope__$simpson_reciprocal,
  swab = high_eater_swab_pred
)

high_eater_swab_both %>% 
  ggscatter(x = 'real', y = 'swab', xlab = 'Real diversity', ylab = 'Predicted diversity after swabbing',
            title = 'Top eaters',
            add = "reg.line", 
             add.params = list(color = "blue", fill = "lightgray"), # Customize reg. line
   conf.int = TRUE, # Add confidence interval
...
low_eater_swab_both <- tibble(
  real = bote$simpson_reciprocal,
  swab = low_eater_swab_pred
)
