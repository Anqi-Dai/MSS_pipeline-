...
prior_pred <-pp_check(fitPrior, type = "boxplot", nreps = 10, notch = FALSE)
pp_check(fitPrior)
```
...
post_pred <- pp_check(fit, type = "boxplot", nreps = 10, notch = FALSE)
pp_check(fit) 
```
...
```{r}
S3 <-  plot_grid(prior_pred,post_pred,
                labels = c('A','B'),
                 nrow = 2) 
...
ggsave('../data/S3_pp_check_173.pdf',
