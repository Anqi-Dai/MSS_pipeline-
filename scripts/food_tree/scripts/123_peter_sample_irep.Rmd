...
them <- read_csv('../data/sum of sugar intake vs max zscore family(E,E).csv') %>% 
  select(sampleid, samplebin, iRep, genus, sum_sugar, iRep_zscore, pid) %>% 
...
them %>% 
  ...
  ggpubr::ggscatter(x = 'ave_sugar', y = 'iRep_zscore',  color = 'genus',palette = 'lancet',
                    xlab = 'Fruits and sweets intake averaged over prior 2 days',
                    ylab = 'Growth rate (standardized within genus) ',
...
ggsave('../data/123_new_2fam_fig3top.pdf', height = 3.5)
