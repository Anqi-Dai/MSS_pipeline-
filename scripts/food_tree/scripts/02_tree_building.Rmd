...
```{r}
cts <- read_tsv('~/pipeline/scripts/picrust2/data/doris57/picrust2_out/KO_metagenome_out/pred_metagenome_unstrat.tsv') %>% 
  column_to_rownames('function') %>% 
  as.matrix()

colsum <- colSums(cts)

normcts <- cts/colsum*1000000

validate <- colSums(normcts)
