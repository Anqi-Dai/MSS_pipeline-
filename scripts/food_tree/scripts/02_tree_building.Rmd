...
```{r}
test <- data.tree::ToDataFrameTree(foodTree)

test %>% 
  kable(caption = 'Food tree in data frame') %>% 
  kable_styling(full_width = F) %>% 
  save_kable(file = "../data/source/food_tree_df.html", self_contained = T)
   
final.table %>% 
  filter(Main.food.description == 'Potato__hash_brown__NFS')
 
treedf <- test  %>% 
  filter(!str_detect(levelName, 'L1|L2|L3|L4|L5|L6'))
```
