...
meta_blautia_fiber %>% 
  mutate(ave_fiber_log = log10(ave_fiber + 0.01)) %>% 
    ggscatter(x = 'ave_fiber_log', y = 'blautia_relab_log', alpha = 0.7,
           ...
           conf.int = TRUE, # Add confidence interval
           cor.coef = TRUE, # Add correlation coefficient.
           cor.coeff.args = list(method = "pearson",  label.y = -5, label.sep = "\n")) 
...
combined %>% 
  ...
  dplyr::select(simpson_reciprocal_log, blautia_relab_log, Enterococcus_relab_log, ave_fiber_log, ave_cal) %>% 
  ...
  gather('xaxis', 'number', ave_cal:ave_fiber_log) %>% 
  ggscatter(x = 'number', y = 'value', alpha = 0.3,xlab = '', ylab = '',
             add = "reg.line",  # Add regressin line
           add.params = list(color = "blue", fill = "lightgray"), # Customize line
           conf.int = TRUE, # Add confidence interval
           cor.coef = TRUE, # Add correlation coefficient.
  ...
  ggsave('../figs/marcel_figures.pdf', width = 6, height = 8)
```
