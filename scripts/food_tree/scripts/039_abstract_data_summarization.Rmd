...
```{r}
box2 <- meta %>% 
  select(fg_ndrink, fg_fruit, timebin)

grp2 <- box2 %>% 
  gather('grp', 'gram', fg_ndrink:fg_fruit) %>% 
  ggboxplot(x = 'timebin', y = 'gram', color = 'grp', add = 'jitter', palette = 'nejm', alpha = 0.6) +
  scale_y_sqrt() 
...
box_combined <- plot_grid(grp2, food_box, stool_box, 
          nrow = 3, 
          align = 'hv',
          #rel_heights = c(1,1),
          axis = 'b')  +
