...
ggsave('../figs/mean_irep_5_genus.pdf', width = 9, height = 28, plot = withoutlier_list)
...
combined %>% 
  ...
  summarise(ave_irep = mean(aveirep)) %>% 
  inner_join(meta) %>% 
  ggscatter(x = 'fg_fruit', y = 'ave_irep', title = 'Enterococcus only',
          add = "reg.line",  # Add regressin line
           add.params = list(color = "blue", fill = "lightgray"), # Customize line
           conf.int = TRUE, # Add confidence interval
           cor.coef = TRUE, # Add correlation coefficient.
...
ggsave('../figs/paper/fruit_enterococcus.jpg', width = 3)
```
...
ggsave('../figs/paper/fruit_no_enterococcus_ave.jpg', width = 5)
```
...
ggsave('../figs/paper/sweets_enterococcus.jpg', width = 5)
```
```{r}
combined %>% 
  filter(!str_detect(best_species, 'Enterococcus')) %>% 
  ...
  summarise(ave_irep = mean(aveirep)) %>% 
  inner_join(meta) %>% 
  ggscatter(x = 'fg_sweets', y = 'ave_irep', title = 'Average of all taxa excluding Enterococcus',
          add = "reg.line",  # Add regressin line
           add.params = list(color = "blue", fill = "lightgray"), # Customize line
           conf.int = TRUE, # Add confidence interval
           cor.coef = TRUE, # Add correlation coefficient.
...
ggsave('../figs/paper/sweets_no_enterococcus_ave.jpg', width = 5)
```
