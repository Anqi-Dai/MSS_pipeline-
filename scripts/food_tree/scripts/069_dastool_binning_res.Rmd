...
drep <- read_csv('../data/growth/quality_metrics_filtered.csv', col_names = F)
...
tb <- read_tsv('../data/growth/bhatpipeline/binning_table_all_simple.tsv') %>% 
  ...
  mutate(drepid = str_glue('{Sample}__concat__{Bin}.fa')) %>% 
  filter(drepid %in% drep$X1)
...
combined %>% 
  ...
  ggsave('../figs/alpha_Enterococcus_irep_no_abx.pdf', width = 13, height = 10)
...
combined %>% 
  ...
  ggsave('../figs/alpha_Enterococcus_irep.pdf', width = 13, height = 10)
```
...
df %>% 
  ...
  ggscatter(x = 'gram', y = 'ave_irep', 
          add = "reg.line",  # Add regressin line
           ...
           cor.coeff.args = list(method = "pearson",  label.sep = "\n")) +
  facet_wrap(grp ~ ., scales = 'free') +
  ggsave('../figs/alpha_Enterococcus_mean_irep.pdf', width = 13, height = 10)
```
