...
# the table that has the info of which samples have to be censored(removed)
censor_samples <- read_csv('../data/cleaned_stool_data/abx_censor_samples_67.csv')

# the table that also has nutrients data
# this is NOT censoring the abx samples 
both_metric <- read_csv('../data/finalized/paired/pair_p3day_food_group_n_nutrients_res.csv') 



# ADD THE stool sample day relative to transplant cuz that is an important factor
stb_this <- read_csv('../data/finalized/stool/filter_stool_close_to_diet_meta.csv') %>% 
...
pheno <-  both_metric %>% 
  ...
  # include only the qualified samples(censored)
  filter(sampleid %in% rownames(cts_raw_top)) %>% 
  ungroup() %>% 
  ...
  full_join(stb_this, by = 'sampleid') %>% 
  column_to_rownames('sampleid') 
```
...
text(pair3.cca2, display="bp", col="red", cex = 0.9)
dev.off() 
...
pheno_nut <- pheno %>% 
  select(all_of(nutrients_names), age_at_hsct, sex, admit_bmi, intensity, drt)
...
```{r}
# the whole model
anova(pair3.cca_nut2, step=1000) 

# axis
anova(pair3.cca_nut2, by="axis", step=1000) 

# individual predictors
anova.cca(pair3.cca_nut2, step=1000, by="margin")

pair3.cca2_margin <- anova.cca(pair3.cca_nut2, step=1000, by="margin")
