...
drop1(pair3.cca, test="perm")

# drop legume_total
pheno_update <- pheno %>% 
  select(-source )

pair3.cca <- cca(cts_raw_top ~ ., pheno_update) 
vif.cca(pair3.cca)

drop1(pair3.cca, test="perm")

pheno_update2 <- pheno_update %>% 
  select(-grain_total )

pair3.cca <- cca(cts_raw_top ~ ., pheno_update2) 
vif.cca(pair3.cca)
...
#
pheno_update3 <- pheno_update2 %>% 
  select(-meat_total, -fats_total, -Sugars_total, -milk_total,-Fibers_total,-Proteing_total, -Fat_total )

pair3.cca <- cca(cts_raw_top ~ ., pheno_update3) 
vif.cca(pair3.cca)
...
plot(pair3.cca)# Each possible factor is plotted at the centroid of the samples on that type, shown as an "X". To find out which one is which, look at last element of the summary of the cca object.
...
cca.step.forward <- ordistep(cca(cts_raw_top ~ 1, data=pheno_update3),
scope=formula(pair3.cca), direction="forward", pstep=1000) # remove legume_total
```

```{r}
# parsimonious 
cca.pars <- cca( cts_raw_top ~ Sodium_total + intensity + Carbohydrates_total +      admit_bmi + age_at_hsct + sex + veggie_total + sugar_total +      fruit_total + egg_total  , data=pheno_update3)
summary(cca.pars)
```


```{r}
# display the species and the quantitative predictors : the nutrients level
plot(cca.pars, scaling = 'species', display=c("species", "bp"))
