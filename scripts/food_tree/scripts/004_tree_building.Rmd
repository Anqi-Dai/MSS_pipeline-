...
uniq_food <- read_csv('../data/cleaned_diet_data/all_food_nsc_with_code.csv') %>% 
  select(-Food_NSC) %>% 
  distinct()  
...
final_table_name <- final.table%>% 
  mutate(newickstring_name = str_glue('foodtreeroot/{L1}/{L2}/{L3}/{L4}/{L5}/{L6}/{Main.food.description}'))
