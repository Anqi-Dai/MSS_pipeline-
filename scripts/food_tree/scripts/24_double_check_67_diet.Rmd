...
# decided to manually corrected for those above
outlier_to_be_corrected %>% 
  write_csv('../data/source/outlier_to_be_corrected_angel.csv')

cereal <- outlier_to_be_corrected %>% 
  filter(Food_NSC == 'Cereal, Cheerios') %>% 
  mutate(ratio = Por_eaten/3) %>% 
  mutate(Calories_kcal = 300,
          Proteing_g = 9,
          Fat_g = 6, 
          Carbohydrates_g = 60, 
          Fibers_g = 9,
...
```{r}
# combine the corrected ones
other <- read_csv('../data/source/outlier_to_be_corrected_angel.csv') %>% 
  mutate(Date = mdy(Date))

all_outlier <- bind_rows(
  other,
  cereal
)
```
