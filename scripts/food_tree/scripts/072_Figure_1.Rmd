...
stool_alpha <- meta %>% 
  ggscatter(x = 'sdrt', y = 'simpson_reciprocal', alpha = alpha_val, size = point_size, 
            ...
            add.params = list(color = "black", fill = "darkblue", size = 1)) +
...
p1099 <- asv_counts_ag %>% 
  filter(str_detect(oligos_id, 'pool1099')) %>% 
  filter(str_detect(sampleid, 'FMT'))

asv_alpha_diversity_ag %>% 
   filter(str_detect(oligos_id, 'pool1099'))  %>% 
  select(sampleid) %>% 
  write_csv('../data/pool1099_sampleid.csv')
``` 
