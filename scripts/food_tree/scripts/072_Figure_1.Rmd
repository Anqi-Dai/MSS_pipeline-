...
fig1_umap <- df %>% 
  arrange(desc(n)) %>% 
  ggscatter(x = 'taxumap-LLLL-1', y = 'taxumap-LLLL-2', color = 'fg1_dom', alpha = 0.2,size = point_size , xlab = 'UMAP1', ylab = 'UMAP2') +
...
```{r top}
# middle right
top_right <- plot_grid(  fig1_umap,fg_legend_,
                  ncol = 1, labels = c('B',''), rel_heights = c(0.5,0.5))

top_left <- plot_grid( tree,
                 nrow = 1,  labels = c('A'))

top <- plot_grid(  top_left,top_right,
                  ncol = 2, rel_widths = c(0.8,0.5))
top
```
...
ggsave('../figs/paper/F1_overview_072_Jan21.pdf',
       width = 190,
       height = 180,
         #height = 60,  
         units = c("mm"),
