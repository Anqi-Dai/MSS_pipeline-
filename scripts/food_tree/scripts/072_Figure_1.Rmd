...
diet_hist <- dtb %>% 
  ...
  gghistogram(x = 'fdrt', xlab = 'Transplant day', ylab = 'Meals recorded', color = 'white', fill = diet_line_color, alpha = 1)+
    scale_x_continuous( breaks = seq(-10, 50, 20)) + 
  annotate("text", x = 5, y = 900, label = "173 patients,\n9,419 total meals\n40,702 food items", hjust = 0, size = 1.5) +
  theme_classic(base_size = 8) +
  theme(aspect.ratio=1,axis.line=element_line(size=.5),
...
stool_alpha <- meta %>% 
     ggplot() +
  rasterise(geom_point(aes(x = sdrt, y = simpson_reciprocal, shape = 'triangle'), alpha = 0.01,size = point_size , shape = 16), dpi = 300) +
...
ggsave('../data/072_stool_line.pdf',width = 40, height = 40, units = "mm", device = 'pdf', 
      plot = stool_alpha, 
      dpi = 300 )
```
...
cal_time <- food %>% 
  mutate(daycal = daycal/1000) %>%
        ggplot() +
  rasterise(geom_point(aes(x = fdrt, y = daycal, shape = 'triangle'), alpha = 0.01,size = point_size , shape = 16), dpi = 300) +
  geom_smooth(aes(x = fdrt, y = daycal), method = "loess", formula = 'y ~ x', colour = diet_line_color ,  size = 1, fill = 'hotpink') +
  labs( x = 'Transplant day',title = '', y = 'Caloric intake\n(X10e3 Kcal)') +
  scale_x_continuous( breaks = seq(0, 50, 20)) +
  theme_classic(base_size = 8) +
  theme(axis.text=element_text(size=axis_text_size),
...
dietalpha_time <- food %>% 
  mutate(faith_pd = faith_pd/1000) %>%
       ggplot() +
  rasterise(geom_point(aes(x = fdrt, y = faith_pd, shape = 'triangle'), alpha = 0.01,size = point_size , shape = 16), dpi = 300) +
  geom_smooth(aes(x = fdrt, y = faith_pd), method = "loess", formula = 'y ~ x', colour = diet_line_color ,  size = 1, fill = 'hotpink') +
  labs( x = 'Transplant day',title = '', y = 'Diet alpha diversity\n(X10e3)') +
  scale_x_continuous( breaks = seq(0, 50, 20)) +
...
```{r}
two_foodlines <- plot_grid(cal_time, NA, dietalpha_time, 
                      rel_heights = c(1, -0.3, 1) ,
          ncol = 1, hjust = 0.5,
          align = 'hv',
          axis = 'tblr')
two_foodlines
ggsave('../data/072_two_foodlines.pdf',width = 30, height = 60, units = "mm", device = 'pdf', 
      plot = two_foodlines, 
      dpi = 300 )
```
...
two_hist <- plot_grid(diet_hist, NA, stool_hist, 
          ...
          axis = 'tblr')
two_hist
ggsave('../data/072_two_hist.pdf',width = 30, height = 60, units = "mm", device = 'pdf', 
      plot = two_hist, 
      dpi = 300 )
```
...
```{r}
# top row 
top_row <- plot_grid(NA, NA, maps,
          nrow = 1, 
          align = 'hv',rel_widths = c(.6,1.5,1.5), 
          labels = c('A', 'B', NA),
          axis = 'tblr')
top_row
```
...
```{r middle_row}
middle_row <- plot_grid(NA,m_panel, fg_panel, NA,
          nrow = 1, 
          align = 'hv',
          rel_widths =  c(.6,1.2,1.7,.6),
          labels = c('G', 'I', 'J', "K"),
          axis = 'tblr')
...
ggsave('../data/F1_overview_raw_072.pdf',
