...
```{r stool_alpha}
stool_alpha <- meta %>% 
  ggscatter(x = 'sdrt', y = 'simpson_reciprocal', alpha = 0.01, size = point_size,
...
cal_time <- food %>% 
  ...
  ggscatter('fdrt', 'daycal',
             alpha = 0.01,size = point_size,
            color = scatter_col,
            ylab = 'Caloric intake\n(X10e3 Kcal)',
            xlab = 'Transplant day',
             add = "loess", conf.int = TRUE,
  ...
  theme_classic() +
  theme(axis.text=element_text(size=axis_text_size),
        axis.title=element_text(size=axis_title_size),axis.title.x = element_blank(),
        axis.text.x = element_blank(),
...
m_panel <- dailymacro %>% 
   ggplot() +
  rasterise(geom_point(aes(x = fdrt, y = eachsum), alpha = 0.009,size = point_size , shape = 16), dpi = 300) +
  geom_smooth(aes(x = fdrt, y = eachsum), method = "loess", colour = diet_line_color ,  size = 1, fill = 'hotpink') +
...
two_hist <- plot_grid(diet_hist, stool_hist, 
          nrow = 2, hjust = 0.5,
          align = 'hv',
          labels = c('A'),
          axis = 'tblr')
...
```{r}
# top row 
top_row <- plot_grid(two_hist, NA, maps,
          nrow = 1, 
          align = 'hv',rel_widths = c(.8,1.5,1.5), 
          labels = c(NA, 'B', NA),
          axis = 'tblr')
top_row
```
...
```{r middle_row}
stool_two <- plot_grid(NA,stool_alpha,
                  nrow = 2,axis = 'lbrt', align = 'hv') 

middle_food2 <- plot_grid(cal_time,dietalpha_time,
          nrow = 2, 
          align = 'hv',
          labels = c( 'G', "H"),
          axis = 'tblr')
...
middle_row <- plot_grid(NA,m_panel, fg_panel, NA,
          nrow = 1, 
          align = 'hv',
          rel_widths =  c(.8,1.2,1.7,.8),
          labels = c(NA, 'I', 'J', "K"),
          axis = 'tblr')

# assemble all
f1 <-  plot_grid(top_row, middle_row,macro_facets,
                 align = 'hv',axis = 'tblr',
                 rel_heights = c(1,1,1),
                 nrow = 3)
