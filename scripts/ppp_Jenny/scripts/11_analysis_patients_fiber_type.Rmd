...
dailycode %>% 
  ...
  ungroup() %>% 
  inner_join(allpt_gvhd %>% select(mrn, d100_a_gvhd)) %>% distinct(mrn)
```
...
ggsave('../data/11_fiber_all_type_daily_intake_without_TCD_gvh.pdf', width = 15, height = 5)
```
...
ggsave('../data/11_fiber_all_soluble_daily_intake_with_TCD.pdf', width = 6, height = 4)
...
all_soluble %>% 
  ...
  facet_grid(~ soluble, labeller = 'label_both') +
  scale_y_sqrt()
ggsave('../data/11_fiber_all_soluble_daily_intake_without_TCD.pdf', width = 6, height = 4)
```
```{r}
# gvhd yes and no
all_soluble %>% 
...
ggsave('../data/11_fiber_all_soluble_daily_intake_withTCD_gvh.pdf', width = 6, height = 4)
...
ggsave('../data/11_alpha_and_fiber_withTCD.pdf', width = 5, height = 5)
meta %>% distinct(mrn)
...
meta  %>% 
  ...
  ggscatter(x = 'mean_fiber', y = 'ave_div',
           ...
           cor.coeff.args = list(method = "pearson",  label.sep = "\n")) +
  theme(aspect.ratio=1)
ggsave('../data/11_alpha_and_fiber_without_TCD.pdf', width = 5, height = 5)
meta %>% 
  filter(!mrn %in% tcd_pt) %>% distinct(mrn)
```
