...
DFC %>% 
  write_csv('../data/cdr3_rearrangement_all_patients.csv')

```

## Group the cdr3_rearrangement by patients and output for each patient
...
dir.create('../data/per_pt')
# write out the df for each patient and have the patient id in the file name
for(i in names(DFC_pt)) {
  write_csv(DFC_pt[[i]] %>% select(cdr3_rearrangement),
            str_glue('../data/per_pt/{i}_cdr3_rearrangement.csv'))
}

```

## Group the cdr3_rearrangement by organs and output for each organ
...
dir.create('../data/per_organ')
# write out the df for each patient and have the patient id in the file name
for(i in names(DFC_Organ)) {
  write_csv(DFC_Organ[[i]] %>% select(cdr3_rearrangement),
            str_glue('../data/per_organ/{i}_cdr3_rearrangement.csv'))
}
```
...
/Users/daia1/pipeline/scripts/ppp_susan/software/gliph/gliph/bin/gliph-group-discovery.pl \
  --tcr=Pt3_cdr3_rearrangement.csv
```
...
Seems nothing really comes out of it
